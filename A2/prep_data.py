...
# embedding_dict = get_word2vec()
	...
	for sent in crf_ready_data_single:
		new_sent = []
		padded_sent = sent + [{"vector":np.zeros((1,glove_dim)), "pos_tag": "PAD"},{"vector":np.zeros((1,glove_dim)),"pos_tag": "PAD"}]
		...
		for idx in range(len(padded_sent)-2):
			...
			curr_word["prev_pos"] = prev_pos
			curr_word["next_pos"] = next_pos
